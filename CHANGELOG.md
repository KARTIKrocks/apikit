...
- **router** — `DefaultErrorHandler` now includes the `details` field from an `*errors.Error` in the JSON response envelope. `errors.Error.WithDetail`/`WithDetails` and the `response` package's envelope already supported arbitrary error metadata via `details`, but `router`'s own `errorBody` and `handleError` never read `Error.Details`, so any details set on an error returned from a router handler were silently dropped before reaching the client
